...
# mrmorris-build - Epic Breakdown

## Overview

This document provides the complete epic and story breakdown for mrmorris-build (Clianta AI Agent Builder), decomposing the requirements from the PRD, UX Design if it exists, and Architecture requirements into implementable stories.

## Requirements Inventory

### Functional Requirements

**Agent Creation & Configuration (11 FRs)**
...
**AI Copilot & Smart Assistance (11 FRs)**
...
**Agent Execution & Orchestration (12 FRs)**
...
**Workspace, Users & Permissions (10 FRs)**
...
**Templates & Knowledge Base (8 FRs)**
...
**Safety, Governance & Monitoring (18 FRs)**
...
**Starter Template:**
- No starter template required - this is a **brownfield enhancement** to existing production codebase
- Build upon existing Clianta architecture with 70+ Mongoose models, 24 AI agents, OAuth integrations, BullMQ queue

**Technology Stack (Must Integrate With):**
...
**New AI Services Required (4 services):**
1. **AgentCopilotService** - Conversational assistant using Gemini 2.5 Pro with streaming responses
2. **InstructionParserService** - Convert plain English to structured actions using LangChain + Gemini
3. **TestModeService** - Dry-run simulator (no real actions executed)
4. **SmartSuggestionService** - Auto-complete, next-step predictions, variable suggestions

**New Data Models Required (5+ models):**
1. **Agent** - Agent configurations with workspace isolation
2. **AgentExecution** - Execution history and logs
3. **AgentTemplate** - Pre-built and custom templates
4. **AgentCopilotConversation** - Chat history (7-day TTL)
5. **AgentCredit** - AI credit tracking per workspace

**Database Patterns:**
- All models must implement workspace isolation: `{ workspace: ObjectId, ref: 'Project' }`
- All queries must filter by workspace (enforced at ORM level)
- Compound indexes: `{ workspace: 1, [field]: 1 }`
- Follow existing 70+ model patterns for consistency

**API Design:**
- RESTful endpoints following pattern: `/api/workspaces/:workspaceId/agents`
- Socket.io for real-time execution updates
- Server-Sent Events (SSE) for AI Copilot streaming
- Middleware: Authentication, workspace access verification, RBAC enforcement

**Background Jobs (BullMQ):**
- New job types: `agent-scheduled-execution`, `agent-event-trigger`
- Must integrate with existing 8 scheduled jobs
- Upstash optimization: Minimize Redis commands (10K/day limit on free tier)
- Job retry logic with exponential backoff

**Integration Services:**
- Extend existing OAuth integrations (Gmail, LinkedIn, Slack, Google Calendar, Google Sheets)
- Add Apollo.io integration for contact enrichment
- Agent-specific rate limiting per integration
- Circuit breakers to auto-pause agents hitting limits
- Integration health monitoring and quota tracking

**Security Requirements:**
- Prompt injection defense: System prompts isolated, tool whitelisting enforced
- Workspace isolation validated via automated CI/CD testing
- Integration credentials encrypted at rest (AES-256-GCM)
- OAuth token auto-refresh with 99.9% success rate
- RBAC enforced on all API endpoints

**AI Credits System:**
- Track consumption: 1 credit/Copilot message, 2-5 credits/agent execution
- Real-time balance with <5 second lag
- Warning at 80%, pause at 100%, 10% grace period (48 hours)
- Credit pools per workspace

**Circuit Breakers & Rate Limits:**
- Per-agent: Max 100 executions/day, 10 executions/min, 100 emails/day
- Per-workspace: Aggregate limits to prevent quota exhaustion
- Per-integration: Slack 1 req/sec, LinkedIn 100 req/day, Gmail 250 units/sec
- Auto-pause and resume on quota refresh

**From Project Documentation:**

**Existing System Integration:**
- Must integrate with 24 existing AI agents (contactAgent, dealAgent, campaignAgent, etc.)
- Extends SupervisorV2, Complexity Analyzer, Execution Planner multi-agent system
- Reuse existing AIClientMemory for workspace context
- Leverage existing IntegrationCredential model for OAuth tokens
...
**Build & Deployment:**
- Monorepo: `npm run dev` runs frontend + backend concurrently
- Frontend: Vercel deployment with automatic builds
- Backend: Railway deployment with container-based scaling
- Hot reload: Next.js (frontend), nodemon (backend)
- Environment-based configuration (.env files)

**Testing Requirements:**
- Jest configured but not actively used (opportunity to establish patterns)
- Automated workspace isolation testing in CI/CD
- Type safety via TypeScript throughout

**Real-Time Features:**
- Socket.io rooms for workspace-based isolation
- Live execution status updates
- AI Copilot chat interface
- Test Mode preview updates

### FR Coverage Map

**Agent Creation & Configuration:**
- FR1 → Epic 1 - Users can create agents by defining goal, triggers, instructions, restrictions, memory, approval settings
- FR2 → Epic 1 - Users can write agent instructions in natural language
- FR3 → Epic 1 - Users can specify multiple trigger types (manual, scheduled, event-based)
- FR4 → Epic 1 - Users can define agent restrictions to prevent unwanted behaviors
- FR5 → Epic 1 - Users can configure agent memory to track state between executions
- FR6 → Epic 1 - Users can set approval requirements for specific actions
- FR7 → Epic 1 - Users can edit existing agents and update configuration
- FR8 → Epic 1 - Users can delete agents from workspace
- FR9 → Epic 1 - Users can duplicate existing agents to create variations
- FR10 → Epic 1 - Users can set agent status (Draft, Live, Paused)
- FR11 → Epic 1 - Users can pause and resume live agents

**AI Copilot & Smart Assistance:**
- FR12 → Epic 4 - Users can interact with AI Copilot via in-builder chat
- FR13 → Epic 4 - AI Copilot can generate complete agent instructions from plain English descriptions
- FR14 → Epic 4 - AI Copilot can answer questions about automation tasks
- FR15 → Epic 4 - AI Copilot can suggest improvements to agent instructions
- FR16 → Epic 4 - AI Copilot can analyze failed executions and explain errors
- FR17 → Epic 4 - AI Copilot can access workspace context (templates, tags, custom fields)
- FR18 → Epic 4 - Users can see auto-complete suggestions while typing
- FR19 → Epic 4 - Users can see next-step predictions based on current logic
- FR20 → Epic 4 - Users can see variable suggestions based on trigger type and available data
- FR21 → Epic 4 - System can convert natural language descriptions into structured action pills
- FR22 → Epic 4 - Users can accept or reject AI Copilot suggestions

**Agent Execution & Orchestration:**
- FR23 → Epic 3 - System can parse natural language instructions and execute corresponding actions
- FR24 → Epic 3 - System can execute agents based on manual triggers
- FR25 → Epic 3 - System can execute agents based on scheduled triggers (daily, weekly, monthly)
- FR26 → Epic 3 - System can execute agents based on CRM events (contact created, deal updated, form submitted)
- FR27 → Epic 3 - System can execute sequential multi-step workflows
- FR28 → Epic 3 - System can execute conditional logic (if/then in plain English)
- FR29 → Epic 3 - System can pause execution for specified time periods
- FR30 → Epic 3 - System can hand off warm leads to human users
- FR31 → Epic 3 - System can execute actions: Send Email, Send LinkedIn Invitation, Web Search, Create Task, Add/Remove Tag, Update Field, Enrich Contact, Wait
- FR32 → Epic 3 - Users can manually trigger agent execution on demand
- FR33 → Epic 3 (Phase 2) - System can chain multiple agents together (Agent A → Agent B)
- FR34 → Epic 3 - System can track agent execution history and status

**Testing & Validation:**
- FR35 → Epic 2 - Users can run agents in Test Mode (dry run) before going live
- FR36 → Epic 2 - System can simulate execution without performing real actions
- FR37 → Epic 2 - Users can select specific contacts/deals to test against
- FR38 → Epic 2 - System can display step-by-step preview of actions
- FR39 → Epic 2 - System can show estimated execution time and AI credit cost for tests
- FR40 → Epic 2 - System can validate instructions and warn about potential errors
- FR41 → Epic 2 - System can return test results within 10 seconds for 80% of runs

**Integration Management:**
- FR42 → Epic 5 - Users can authenticate third-party integrations via OAuth
- FR43 → Epic 5 - System can manage OAuth token refresh automatically
- FR44 → Epic 5 - System can notify users when credentials expire
- FR45 → Epic 5 - Users can connect Gmail for emails and inbox
- FR46 → Epic 5 - Users can connect LinkedIn for invitations and messages
- FR47 → Epic 5 - Users can connect Apollo.io for contact enrichment
- FR48 → Epic 5 - Users can connect Slack for messages and notifications
- FR49 → Epic 5 - Users can connect Google Calendar for availability and events
- FR50 → Epic 5 - Users can connect Google Sheets for data import/export
- FR51 → Epic 5 - System can access internal CRM data (contacts, deals, tasks, tags, custom fields)
- FR52 → Epic 5 - System can enforce rate limits per integration (Slack 1 req/sec, LinkedIn 100 req/day, etc.)
- FR53 → Epic 5 - System can display integration health status and quota usage

**Workspace, Users & Permissions:**
- FR54 → Epic 7 - System can isolate data between workspaces (zero cross-workspace access)
- FR55 → Epic 7 - Workspace Owners can create, edit, delete any agent in workspace
- FR56 → Epic 7 - Workspace Admins can create, edit, delete any agent in workspace
- FR57 → Epic 7 - Workspace Members can view agents and trigger manual runs only
- FR58 → Epic 7 - Workspace Viewers can view agents and logs in read-only mode
- FR59 → Epic 7 - Workspace Owners can manage users and assign roles
- FR60 → Epic 7 - Agent creators can set edit permissions on individual agents
- FR61 → Epic 7 - Agent creators can limit which integrations specific agents can access
- FR62 → Epic 7 - System can cascade workspace-level integration permissions to agents
- FR63 → Epic 7 - System can display clear error messages for permission issues

**Templates & Knowledge Base:**
- FR64 → Epic 6 - Users can browse pre-built agent templates
- FR65 → Epic 6 - Users can install templates and customize for their needs
- FR66 → Epic 6 - System can provide 10 pre-built templates for common sales workflows
- FR67 → Epic 6 - Users can save their own agents as templates for reuse
- FR68 → Epic 6 - Users can share templates with team members in workspace
- FR69 → Epic 6 (Phase 3) - Users can browse public agent marketplace
- FR70 → Epic 6 (Phase 3) - Users can publish agents to marketplace
- FR71 → Epic 6 (Phase 3) - Users can rate and review marketplace agents

**Safety, Governance & Monitoring:**
- FR72 → Epic 7 - System can track AI credit consumption per execution
- FR73 → Epic 7 - System can display real-time AI credit balance
- FR74 → Epic 7 - System can warn users at 80% credit usage
- FR75 → Epic 7 - System can pause agents when credit quota exhausted
- FR76 → Epic 7 - System can enforce circuit breakers (max 100 executions/day/agent)
- FR77 → Epic 7 - System can enforce email limits (max 100 emails/day/agent)
- FR78 → Epic 7 - System can enforce rate limits (max 10 executions/minute/agent)
- FR79 → Epic 7 - System can defend against prompt injection via system prompt isolation
- FR80 → Epic 7 - System can whitelist allowed tools/actions for security
- FR81 → Epic 3 - System can log all executions with timestamp, user, outcome
- FR82 → Epic 3 - Users can view execution logs for debugging
- FR83 → Epic 3 - Users can export agent data (configurations, logs) in JSON/CSV
- FR84 → Epic 7 - System can retain execution logs per tier (30 days standard, 365 days enterprise)
- FR85 → Epic 7 - System can audit all admin actions (who created/edited/deleted agents, when)
- FR86 → Epic 7 - System can auto-pause agents hitting integration rate limits
- FR87 → Epic 3 - System can display agent execution dashboards with success/failure rates
- FR88 → Epic 7 - System can alert users when agents fail or quotas exhausted
- FR89 → Epic 7 - System can enforce workspace isolation at database level
